...
import 'package:todoeight/utils/src/customization/days_of_week_style.dart';
import 'package:todoeight/utils/src/customization/header_style.dart';
import 'package:todoeight/utils/src/customization/calendar_builders.dart';
import 'package:todoeight/utils/src/calendar_controller.dart';
import 'package:todoeight/utils/src/widgets/cell_widget.dart';
import 'package:todoeight/utils/src/widgets/custom_icon_button.dart';
...
typedef void OnCalendarCreated(DateTime first, DateTime last,
    CalendarFormat format, Map<DateTime, List<dynamic>> events);
...
class TableCalendar extends StatefulWidget {
  ...
  final MyDaysOfWeekStyle daysOfWeekStyle;

  /// Style for `TableCalendar`'s Header.
  final MyHeaderStyle headerStyle;

  /// Set of Builders for `TableCalendar` to work with.
  final MyCalendarBuilders builders;

  TableCalendar({
    Key key,
    @required this.calendarController,
    this.locale,
    this.daysOfWeekStyle = const MyDaysOfWeekStyle(),
    this.events = const {},
    this.holidays = const {},
    this.onDaySelected,
    ...
    this.calendarStyle = const MyCalendarStyle(),
    this.headerStyle = const MyHeaderStyle(),
    this.builders = const MyCalendarBuilders(),
...
class _TableCalendarState extends State<TableCalendar>
  ...
  void didUpdateWidget(TableCalendar oldWidget) {
    ...
    if (oldWidget.events != widget.events) {
      widget.calendarController.myevents = widget.events;
    }

    if (oldWidget.holidays != widget.holidays) {
      widget.calendarController.myholidays = widget.holidays;
    }
  }
      ...
      Expanded(
        child: GestureDetector(
          ...
          child: Padding(
            ...
            child: Text(
                DateFormat.yMMMMEEEEd(widget.locale)
  ...
  Widget _buildTable() {
    ...
    int x = 0;
    while (x < widget.calendarController.myvisibleDays.value.length) {
      children.add(_buildTableRow(widget.calendarController.myvisibleDays.value
          .skip(x)
          .take(daysInWeek)
          .toList()));
      x += daysInWeek;
  ...
  TableRow _buildDaysOfWeek() {
    return TableRow(
      children:
          widget.calendarController.myvisibleDays.value.take(7).map((date) {
        final weekdayString = DateFormat.E(widget.locale).format(date);
        final isWeekend =
